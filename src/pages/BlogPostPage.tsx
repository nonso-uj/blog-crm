...
const BlogPostPage = () => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values: Post) => {
      ...
      try {
        ...
        const updatedData = blogList.map((item: any) =>
          item.id === values.id
            ? {
                ...item,
                ...values,
  ...
  useEffect(() => {
    let currentPost = blogList.filter((curr: any) => curr.id === Number(id));
    console.log(currentPost[0], id, blogList);
    formik.setValues(currentPost[0]);
    setSections(currentPost[0]?.sections);
  }, []);
    ...
    <DashboardLayout>
      <div className="p-5">
        <div className="flex flex-col lg:flex-row justify-start lg:justify-between gap-x-3">
          <h3 className="text-center lg:text-start">Edit blog details</h3>
          ...
          <div className="flex flex-col justify-start gap-y-3">
            <div id="fileUpload" className="max-w-lg">
              ...
              {formik?.values.headerImage && (
                <small className="mt-2">{formik?.values.headerImage}</small>
              )}
              ...
              {sections?.map((curr, index) => (
                ...
                >
                  ...
                  <div id="fileUpload" className="max-w-lg">
                    ...
                    {curr?.picture && (
                      <small className="mt-2">{`${curr?.picture}`}</small>
                    )}
                  </div>
